...
| # | 项 | 修复状态 |
| :-- | :--- | :--- |
| 5 | 接单接口空请求 NPE | ✅ 已修（commit `9e475c0`：`create()` 加空 body/缺字段兜底 + `@Validated` 校验；实测空 body/无 body 均优雅返回 400） |
| 6 | 大厅状态列显示错误 | ✅ 已修（commit `cf298b9`：前端改用公共 `TASK_STATUS` 中文枚举，与后端 webValue 匹配；全站无英文状态字面量显示比较） |
| 7 | 前端方案 A 未全覆盖 | ✅ 已修（2026-08-15 catch 覆盖补全：发布/接单/详情/类型加载等 12 页请求链补 `.catch` + SUCCESS_CODE 检查，失败提示并复位 loading） |
