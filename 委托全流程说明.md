...
- **注册**: `POST /user/register`。新用户注册后**直接可用**，无需邮箱验证（邮箱激活邮件与 `/user/active/{id}` 激活接口已下线）。
- **登录**: `POST /user/login`，返回 JWT，后续请求头携带 `jwt` 令牌。
- **账号状态**: 注册即 `isActive = true` 且 `isEnabled = true`；管理员可通过后台启用/禁用账号（`isEnabled` 为唯一登录门槛之一）。
...
3. **管理员审核**
   ...
   - **API**: 
     - 允许发布: `PUT /admin/task/allowPublish/{id}`
...
6. **确认接收者**
   ...
   - **状态变更**: 
     - 任务状态: `ONGOING` -> `ACCEPTED`
     - 申请记录状态: 选中者的记录变为 `CHECKED`，其余变为 `UNCHECKED`。

### 阶段三：执行与完成 (Execution & Completion)

7. **任务执行（履约节点打卡）**
   - **角色**: 接收者
   - **操作**: 线下或线上完成委托内容，并按**标准化三节点**履约打卡：**已联系 → 已取件 → 已送达**。
   - **API**: `POST /taskUpdate/node`（支持打卡图片、定位、备注）
   - **节点记录**: 写入 `TaskUpdates`，`UpdateType` 为 `CONTACTED / PICKED_UP / DELIVERED`。
   - **历史兼容**: 早期自由文本进度（`PROGRESS_UPDATE`，`POST /taskUpdate/add`）记录保留，仅作历史展示，不再作为主推流程。
...
10. **自动评价（定时任务）**
   - 任务完成超过 `app.auto-review.delay-hours`（默认 **24 小时**）仍无人工评价时，系统自动生成中性好评（**3 星**，评价内容标注「系统自动评价」）。
   - 评价人取系统首个管理员账号，`IsApproved = 1`；演示时可将 `delay-hours` 调小快速触发。
